...
const MIN_TOTAL_LOADER_MS = 1400
...
export default function BootLoader({ onComplete }) {
  ...
  const [statusLine, setStatusLine] = useState('Checking Connection')
...
useEffect(() => {
  ...
  const waitForBackend = async () => {
    ...
    while (!cancelled) {
      ...
      attempt += 1
      setStatusLine(
        attempt === 1 ? 'Checking Connection' : `Checking Connection (try ${attempt})`
      )
      await wait(1200)
    }
    return false
  ...
  const run = async () => {
    try {
      ...
      setStatusLine('Checking Connection')
      await wait(140)

      const backendReady = await waitForBackend()
      if (!backendReady || cancelled) return

      setStatusLine('Loading Assets')
      ...
      if (cancelled) return
      setStatusLine('Getting Things Ready')
      ...
      if (cancelled) return
      setStatusLine('Starting Experience')
      await wait(180)
      setProgress(100)
      await wait(180)
      if (cancelled) return
      ...
      if (cancelled) return
      setStatusLine(error.message || 'Loading failed.')
    }
  }
    ...
    <div ref={loaderRef} className={`boot-loader ${isExiting ? 'boot-loader-exit' : ''}`} role="status" aria-live="polite">
      ...
      <div className="boot-loader-progress-dock">
        ...
        <div className={`boot-loader-progress-track ${progressStage}`} aria-hidden="true">
          <span className={`boot-loader-milestone m33 ${progress >= 33 ? 'is-hit' : ''}`}></span>
          <span className={`boot-loader-milestone m66 ${progress >= 66 ? 'is-hit' : ''}`}></span>
          <span className={`boot-loader-milestone m99 ${progress >= 99 ? 'is-hit' : ''}`}></span>
