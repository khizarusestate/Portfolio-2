...
useEffect(() => {
  ...
  const run = async () => {
    try {
      ...
      // ✅ Require backend connection before continuing
      try {
        ...
        if (!networkResponse.ok) {
          throw new Error('Backend is not reachable. Stopping boot.');
        }
      } catch (err) {
        if (cancelled) return;
        throw new Error('Backend request failed. Stopping boot.');
      }
