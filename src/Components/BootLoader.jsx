...
export default function BootLoader({ onComplete }) {
  ...
  const { play, activateExperience } = useSound()
  ...
  useEffect(() => {
    ...
    const run = async () => {
      try {
        ...
        if (cancelled) return
        setStatusLine('Ready. Click to continue.')
        setProgress(100)
        setIsReadyToContinue(true)
      } catch (error) {
        if (cancelled) return
        setProgress(100)
        setStatusLine((error.message || 'Loading failed.') + ' Click to continue.')
        setIsReadyToContinue(true)
      }
    }
  ...
  }, [isDark, play, userTextRegistry])

  const handleContinue = async () => {
    if (!isReadyToContinue || isContinuing) return

    setIsContinuing(true)
    setStatusLine('Launching experience...')
    await activateExperience({ mode: isDark ? 'dark' : 'light' })
    setIsExiting(true)
    await wait(460)
    onComplete?.()
  }
      ...
      <div className="boot-loader-progress-dock">
        ...
        {isReadyToContinue && (
          <button
            ...
            className="boot-loader-continue"
          >
            {isContinuing ? 'Launching...' : 'Click to continue'}
          </button>
        )}
      </div>
