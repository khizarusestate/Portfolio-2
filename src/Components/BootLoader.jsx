...
export default function BootLoader({ onComplete }) {
  ...
  const [statusLine, setStatusLine] = useState('Loading Assets')
...
useEffect(() => {
  ...
  const run = async () => {
    try {
      ...
      await animateProgress(0, 40, 520, (value) => {
      ...
      await animateProgress(40, 75, 560, (value) => {
        if (!cancelled) setProgress(value)
      })

      if (cancelled) return
      setStatusLine('Final Touches')
      await preloadImage(ASSET_IMAGE)

      if (cancelled) return
      await animateProgress(75, 99, 620, (value) => {
        if (!cancelled) setProgress(value)
      })
